...
import { homeDateAnswer } from "@/lib/scavengerConfig";
...
// Rotation in degrees (positive = right, negative = left) - varied for each heart
...
// Filter out hearts in the center content area (behind title, form, etc.)
const HEARTS_OUTSIDE_CONTENT = HEARTS.map((heart, i) => ({ heart, origIndex: i }))
  .filter(({ heart }) => {
...
export default function Home() {
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center px-4 overflow-hidden">
      {/* Baby pink background - base layer */}
      <div className="fixed inset-0 bg-[#F5C6C6] -z-20" aria-hidden />
      {/* Yellow heart emojis - above background, below content */}
      <div
